...
void CMyApp::InitSkyBox()
{
	...
	// skybox texture
		//ez egy cubemap lesz, minden oldalát külön-külön kell megadni
	...
	// a GL_TEXTURE_MAG_FILTER-t és a GL_TEXTURE_MIN_FILTER-t beállítja az AttachFromFile
		//három textúra koordinátája lesz...?
...
void CMyApp::InitShaders()
{
	...
	// shader program alternatív rövid létrehozása, egyetlen függvényhívással a fenti három:
	m_programSkybox.Init(
		...
		//linkProgram automatikusan meghívódik
	);

	m_programAxis.Init(
		{
			{ GL_VERTEX_SHADER, "axis2.vert" },
			{ GL_FRAGMENT_SHADER, "axis2.frag" }
		}
	);
}
...
void CMyApp::Render()
{
	...
	glm::mat4 suzanneWorld = glm::translate( glm::vec3(0,-2,0));
	m_program.Use();
	...
	// most kisebb-egyenlőt használjunk, mert mindent kitolunk a távoli vágósíkokra
		//alapból a glClear()-kor mindent beállítunk 1-re
	glDepthFunc(GL_LEQUAL);

	m_SkyboxVao.Bind();
	m_programSkybox.Use();
	//eltolás kamerával: kocka középpontja mindig a kamera legyen
