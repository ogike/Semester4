...
bool CMyApp::Init()
{
	...
	// mesh betoltese
	m_mesh = ObjParser::parse("drone.obj");
	m_mesh->initBuffers();
...
void CMyApp::DrawGround()
{
	...
	glUniform1i(glGetUniformLocation(m_programID, "isSuzanne"), 0);
...
void CMyApp::DrawMesh()
{
	...
	glUniform1i(glGetUniformLocation(m_programID, "isSuzanne"), 1);
	glUniform1i(glGetUniformLocation(m_programID, "showTex"),  (showTex) ? 1 : 0 );
	glUniform4fv(glGetUniformLocation(m_programID, "Kd"), 1, &suzCol.x);
	...
	//NOTE: ez amivel tényleg kirajzoljuk
	m_mesh->draw();
...
void CMyApp::Render()
{
	...
	//új ablak létrehozása
		//benne lévõket csak akkor jelenítjük meg, ha le van nyitva az ablak
	if (ImGui::Begin("Test")) {

		//mivel nincs megadva milyen ablakba jelenítse, by default Debug ablakba pakolja
		ImGui::Text("Eye:\n x: %f,\n y: %f,\n z: %f", m_eye.x, m_eye.y, m_eye.z);
		ImGui::Text("At:\n x: %f,\n y: %f,\n z: %f", m_at.x, m_at.y, m_at.z);
		ImGui::Text("Up:\n x: %f,\n y: %f,\n z: %f", m_up.x, m_up.y, m_up.z);

		//input csúszka készítése
		ImGui::SliderFloat3("Eye", &m_eye.x, -5, 5);
		ImGui::SliderFloat3("At", &m_at.x, -5, 5);
		ImGui::SliderFloat3("Up", &m_up.x, -5, 5);

		//ha rákattintottunk, ez igaz lesz
			//ez csak egy frame-ben lesz igaz
		if (ImGui::Button("Toggle Mesh")) {
			draw = !draw;
		}

		//ezt saját maga kezeli a bool változót
		if (ImGui::Checkbox("Show Suzanne", &draw));
		if (ImGui::Checkbox("Show Suzanne Texture", &showTex));

		ImGui::SliderFloat4("Color", &suzCol.x, 0, 1);
