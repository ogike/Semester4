...
GLuint CMyApp::GenerateRandomTexture()
{
	...
	unsigned char tex[W][H][3]; //3: rgb csatornák
	...
	glTexImage2D(GL_TEXTURE_2D,	// melyik binding point-on van a textúra erőforrás, amihez tárolást rendelünk
		...
		GL_RGB,					// textúra belső tárolási formátuma (GPU-n) (lehetne pl GL_GRB, stb)
	...
	//nagyítás esetén milyen szűrőt használjon
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	//kicsinítéshez esetén milyen szűrőt használjon: mipmap-est
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST_MIPMAP_LINEAR);
	//mi legyen, ha vízszintesen kilépünk a [0,1] tartományból
	//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	//mi legyen, ha függőlögesen ...
...
void CMyApp::Update()
{
	...
	if (w) { //ha w le van nyomva, előre megyünk
		m_eye += 0.1f * forward;
	...
	//jobbra/balra mozgás
									//a kettőre merőleges vektort ad vissza
...
void CMyApp::Render()
{
	...
	//extra feladat: 
	glUniform1f(glGetUniformLocation(m_programID, "time"), SDL_GetTicks() / 1000.f);
...
void CMyApp::MouseMove(SDL_MouseMotionEvent& mouse) {
	//mouse.state: épp milyen gombok voltak lenyomva, bitek sorozataként
		//(0: nincs lenyomva, 1: lenyomva)
		//bit mask-ot rátesszük, ha bal egér gomb le van nyomva, igazat ad
	if (mouse.state & SDL_BUTTON_LMASK) {
		m_theta -= mouse.xrel * 0.005;
		m_fi += mouse.yrel * 0.005;
		//legfeljebb fölfele/lefelé nézzünk, ne "billenjünk túl" (megfordítana mindent)
